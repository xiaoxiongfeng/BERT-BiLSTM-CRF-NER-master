    ...
    def _read_data(cls, input_file):
        ...
        for line in lines:
            try:
                ...
                print("line")
                j += 1
            l = ' '.join([label for label in slots if len(label) > 0])
            w = ' '.join([word for word in sentence if len(word) > 0])
            ans.append([l, w])
        print(i, j)     #14434318 3334563
        return ans
    def _read_print(cls, input_file):
        if input_file is None:
            assert 'path empty'
        #try:
        with open(input_file, mode='r', encoding='utf-8') as fr:
        ...
        print(i, j)     #14434318 3334563
        return ans
    #def _read_print_(cls, input_file):

class NerProcessor(DataProcessor):      #在这里需要更改训练、测试等工作！
    def get_train_examples(self, data_dir):
        return self._create_example(     #这里的参数是返回值
            #self._read_data(os.path.join(data_dir, "train.txt")), "train"
            self._read_data(os.path.join(data_dir, "dianbo_mytrain.txt")), "train"
        )

    def get_dev_examples(self, data_dir):
        return self._create_example(
            self._read_data(os.path.join(data_dir, "dianbo_myeval.txt")), "dev"
        )

    def get_test_examples(self, data_dir):
        return self._create_example(
            self._read_data(os.path.join(data_dir, "dianbo_texts_parse1")), "test")

    def get_print_examples(self, data_dir):
        return self._create_print(
            self._read_print(os.path.join(data_dir, "print.txt")), "print")
    ...
    def _create_example(self, lines, set_type):   #在这里出现了问题了
        examples = []
        for (i, line) in enumerate(lines):
            guid = "%s-%s" % (set_type, i)
            text = tokenization.convert_to_unicode(line[1])
            print("myline0", line[0])
            label = tokenization.convert_to_unicode(line[0])
            print("mylabel", label)
            #if i == 0:
                #print("line0", line, label)
            examples.append(InputExample(guid=guid, text=text, label=label))
        return examples

    def _create_print(self, lines, set_type):   #在这里出现了问题了
        examples = []
        for (i, line) in enumerate(lines):
            guid = "%s-%s" % (set_type, i)
            text = tokenization.convert_to_unicode(line)
            print("myline0", line)
            vir = "I_videoName " * len(line.split(" "))
...
#         raise ValueError("At least one of `do_train` or `do_eval` must be True.")
    ...
    if FLAGS.do_train:
        # 1. 将数据转化为tf_record 数据,所以这里会根据前面的生成结果判断需不需要重新转化
        ...
        train_input_fn = file_based_input_fn_builder(     #这里承接上一步filed_based_convert_examples_to_features的结果
            input_file=train_file,
            seq_length=FLAGS.max_seq_length,
            is_training=True,
            drop_remainder=True)
        #也就是说以上返回的是一个函数，即input_fn的函数
        #训练模型
    ...
    if FLAGS.do_predict:
        ...
        with codecs.open(output_eval_file, "w", encoding='utf-8') as writer:
            ...
            for key in sorted(predicted_result.keys()):
                ...
                print(key, str(predicted_result[key]))
    ...
    if FLAGS.do_print:
        ...
        predict_examples = processor.get_print_examples(FLAGS.data_dir)
        predict_file = os.path.join(FLAGS.output_dir, "predict.tf_record")
        filed_based_convert_examples_to_features(predict_examples, label_list,
                                                 FLAGS.max_seq_length, tokenizer,
